...
Product/category search is powered by `@rokmohar/medusa-plugin-meilisearch`, registered in `apps/backend/medusa-config.ts` (`plugins` array). It uses the **single-index `field-suffix` i18n strategy** (`i18n: { strategy: "field-suffix", languages: ["en","es"] }`): every product/category lives in ONE index (`products` / `categories`), and each translatable field gets a language-suffixed sibling (`title_es`, `description_es`) generated by the transformers from Medusa's **native Translation module** (`featureFlags.translation` + `@medusajs/medusa/translation`; helper `apps/backend/src/utils/translations.ts`). Until translations are entered in admin, the `_es` fields are simply absent and Spanish falls back to the default text.

> ⚠️ Do **not** switch back to the plugin's `separate-index` strategy: in v1.4.3 its sync step writes the default-language documents into *both* `<index>_<lang>` indexes and the store route merges across them, so every product comes back **twice** (and Spanish search returns nothing). The single-index strategy makes duplicate results structurally impossible. If you ever change strategy, delete the stale Meilisearch indexes and re-sync.
